...
public class MotionHelper {
	...
	public static Vec3 getVelocity(ServerPlayer player) {
		return VELOCITIES.get(player.getUUID());
	}
	...
	public static void update(ServerPlayer player) {
		...
		Long newTime = player.serverLevel().getGameTime();
		...
		if (oldVel == null) {
			//System.out.println("Error: old value for velocity is null, setting it to 0.");
			oldVel = new Vec3(0, 0, 0);
		}
		// Set old time to the current time if it's null.
		if (oldTime == null) {
			//System.out.println("Error: old value for time is null, setting to current time.");
			oldTime = newTime;
		}
