...
public class HailstoneItem extends Item {
	...
	public HailstoneItem(Properties properties) {
		super(properties);
	}
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
		...
		if (!level.isClientSide) {
			HailstoneEntity hailstone = new HailstoneEntity(level, player);
			hailstone.setItem(itemstack);
			hailstone.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 1.5F, 1.0F);
