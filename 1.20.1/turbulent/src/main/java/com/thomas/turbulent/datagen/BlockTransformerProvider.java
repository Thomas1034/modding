...
public abstract class BlockTransformerProvider implements DataProvider {
	...
	public CompletableFuture<?> run(CachedOutput cache) {
		...
		if (!this.transformers.isEmpty())
			return this.save(cache, this.output.getOutputFolder(PackOutput.Target.DATA_PACK).resolve(this.modid)
					.resolve("block_transformers"));

		// Returns an empty completable future that does nothing.
